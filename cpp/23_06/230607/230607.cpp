...
void Shuffle(int* ptrArray, int arraySize, int shuffleCount)
{
    ...
    for (int i = 0; i < arraySize; i++)
    {
        for (int i = 0; i < shuffleCount; i += 1)
        {
            ...
            randomIdx2 = rand() % 45;
            int temp = 0;
            temp = *(ptrArray+randomIdx1);
            *(ptrArray + randomIdx1)= *(ptrArray+randomIdx2);
            *(ptrArray+randomIdx2) = temp;
        }
    }
...
void Lottery()
{
    ...
    Shuffle(lotteryNumber, (sizeof(lotteryNumber) / sizeof(int)), shuffleCount);
